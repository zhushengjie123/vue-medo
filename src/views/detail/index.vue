  ...
  <div class="detail">
    ...
    <section class="con">
      ...
      <div class="conPoin">
        ...
        <span v-show="categoryView.category4Name">{{
          categoryView.category4Name
        }}</span>
      </div>
      <!-- 主要内容区域 -->
      <div class="mainCon">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="spuSaleAttr in spuSaleAttrList" :key="spuSaleAttr.id">
                ...
                <dd
                  changepirce="0"
                  :class="value.isChecked === '1' ? 'active' : ''"
                  v-for="value in spuSaleAttr.spuSaleAttrValueList"
                  :key="value.id"
                  @click="changeIsChecked(value, spuSaleAttr)"
            ...
            <div class="cartWrap">
              <div class="controls">
                ...
                <a
                  href="javascript:"
                  class="mins"
                  @click="num <= 1 ? (num = 1) : num--"
                  >-</a
                >
              </div>
              <div class="add">
                <!-- 以前路由跳转是从A路由跳转到B路由，现在加入购物车进行路由跳转之前还要发请求,
                将产品信息通过请求的方式通知给服务器，服务器进行相应的存储 -->
...
export default {
  ...
  computed: {
    ...mapState("detail", { goodsInfo: "goodsInfo" }),
    ...mapGetters("detail", ["categoryView", "skuInfo", "spuSaleAttrList"]),
    skuImageList() {
      //为了子组件在获得该props时并使用其上的属性时至少是一个带有空属性的数组
      return this.skuInfo.skuImageList || [{}];
    },
  },
  methods: {
    changeIsChecked(value, spuSaleAttr) {
      //修改颜色和版本，排他算法，点击后其他的isChecked属性变为0，而点击的变为1
      spuSaleAttr.spuSaleAttrValueList.forEach((element) => {
        element.isChecked = "0";
      });
      value.isChecked = "1";
    ...
    // 切换购物车
    //可通过加async 来保证输出的是promise结果
    addToCard() {
      // 1.派发请求,实质是在调用仓库中的addToCard函数方法
      //2.该派发请求在仓库里加上了async，所以其结果一定是一个promise
      //3.将该值返回promise结果，await是用于返回一个promise的处理结果非它本身
      if(this.num!=='')
      this.$store //then和catch方法捕捉错误
        .dispatch("detail/addToCard", {
          skuId: this.$route.params.skuid,
          skuNum: this.num,
        }) 
        .then(
          () => this.$router.push({ name: "addcardsuccess",query:{
            num:this.num
          } }),
          //利用回话储存传参，但是不能传对象，对象需要转化为JSON格式传过去，需要的时候再解析
          //因为addcardsuccess组件只是用于临时展示添加信息只用回话储存就可以了
          sessionStorage.setItem("addcard", JSON.stringify(this.skuInfo))
        )
        .catch((error) => {
          throw error.message;
        });
      ...
      //   });
      //   //进行路由跳转
      //   this.$router.push({name:'addcardsuccess'})
      // } catch (error) {
      //   alert(error.message);
      // }
    },
  ...
  watch: {
    num(newValue, oldValue) {
      //利用了es6的Number.isInteger方法来判断是不是整数
      if (
